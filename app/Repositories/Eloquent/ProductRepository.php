...
class ProductRepository extends BaseRepository implements ProductRepositoryInterface
{
    ...
    {
        $product = DB::table('product')->join('categories', 'product.categories_id', '=', 'categories.id')
        ->select('product.*')
        ->where('categories.id', 1)->orwhere('categories.parent_id', 1)->orderby('product.id', 'desc')->take(8)->get();
        return $product;
    }
